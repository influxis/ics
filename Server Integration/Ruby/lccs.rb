...
module RTC
  ...
  DEBUG = false
  VERSION = "$Revision: #7 $ - $Date: 2011/07/20 $"
  ...
  class AccountManager
    public
      ...
      # Create a room.
      #
      # If no template is specified, use the default, empty template.
      # If deleteOnExit is true, delete the room when session end.
      #
...
if __FILE__ == $0
  ...
  begin
	...
	ARGV.clear
      ...
      when "--subscribe-collection" then
	room = ARGV.shift
        collection = ARGV.shift
	nodes = ARGV
	...
	ARGV.clear
      ...
      when "--unsubscribe-collection" then
	room = ARGV.shift
        collection = ARGV.shift
	nodes = ARGV
