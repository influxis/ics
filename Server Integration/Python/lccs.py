...
DEBUG = False
VERSION = "$Revision: #6 $ - $Date: 2011/07/20 $"
...
class accountmanager:
  ...
  def createRoom(self, room, template=None, deleteOnExit=False):
    """
    create a room
    """

    if template == None: template = "default"
    data = [ "mode=xml&room=%s&template=%s" % (room, template) ]
    if deleteOnExit: data.append("&deleteonexit=true")
    if self.__authToken: data.append("&%s" % self.__authToken)

    resp = http_post(self.url, ''.join(data), self.__authHeaders)
    if DEBUG: logging.info(resp)
  ...
  def list(self, type=None):
    ...
    if type == None:
      type = ROOM_ITEMS
    elif type != accountmanager.TEMPLATE_ITEMS and type != accountmanager.ROOM_ITEMS and type != accountmanager.ARCHIVE_ITEMS:
  ...
  def listArchives(self):
    ...
    return self.list(accountmanager.ARCHIVE_ITEMS)
  ...
  def deleteArchive(self, a, list=False):
    """
    delete an archive
    """
    if a == None:
    ...
    return self.delete(a, accountmanager.ARCHIVE_ITEMS, list)
  ...
  def registerRepository(self, endpoint=None, token=None):
    """
    Register endpoint URL for recording repository
    """
    acctid = self.__roomInstance.split("/")[0]
    data = "account=%s&action=registerrepository" % acctid
    if endpoint:
      data += "&endpoint=%s" % urllib.quote_plus(endpoint)
    if token:
      data += "&token=%s" % urllib.quote_plus(token)
  ...
  def unregisterRepository(self):
    ...
    """
    return self.registerRepository()
  ...
  def getRepositoryInfo(self):
    ...
    """
    acctid = self.__roomInstance.split("/")[0]
    resp = http_get("%sapp/rtc?action=repositoryinfo&account=%s&%s" % (self.__baseURL, acctid, self.__authToken), self.__authHeaders)
    if DEBUG: logging.info(resp)
    ...
    for p in params:
      ...
      if p.hasChildNodes():
        ...
        value = None
      if name == 'repositoryEndpoint':
        result['endpoint'] = value
      elif name == 'repositoryToken':
        result['token'] = value

    return result
    
  def startRecording(self, room, archiveId, guestsAllowed = True):
    """
    Start recording a specified room
    """
    itemBody = {'fullSession':True, 'archiveID':archiveId, 'guestsAllowed':guestsAllowed}
    return self.publishItem(room, 'RoomManager', 'roomState', 
    	{ 'itemID' : 'recordingState', 'body' : itemBody } )
    
  def stopRecording(self, room):
    """
    Stop recording a specified room
    """
    return self.retractItem(room, 'RoomManager', 'roomState', 'recordingState')
    
  def moveArchive(self, archive, template = None):
    """
    Move an archive to a specified template
    """
    url = "%s/%s/%s" % (self.__contentURL(), accountmanager.ARCHIVE_ITEMS, archive)
    data = "action=set-property&response=inline&name=cr:description&%s" % (self.__authToken)
    if template:
      data += "&value=%s" % template;
...
if __name__ == "__main__":
  ...
  def usage():
    ...
    print "    --create-autodelete room [template]"
    print "    --delete room"
    ...
    print ""    
    print "    --register-repository endpoint [token]"
    print "    --unregister-repository"
    print "    --repository-info"
    print "    --start-recording room [[archiveId] [guestsAllowed]]"
    print "    --stop-recording room"
    print "    --delete-archive archive"
    print "    --move-archive archive [template]"    
    exit(1)
  ...
  elif args[0] == "--create-autodelete":
    #
    # create room, autodelete when session ends
    #
    template = None
    if len(args) > 2: template = args[2]
    print am.createRoom(args[1], template, True)
  ...
  elif args[0] == "--delete-archive":
    #
    # delete archive
    #
    print am.deleteArchive(args[1])
  ...
  elif args[0] == "--register-repository":
    if len(args) > 2:
      print am.registerRepository(args[1], args[2])
    else:
      print am.registerRepository(args[1])
    
  elif args[0] == "--unregister-repository":
    print am.unregisterRepository()
    
  elif args[0] == "--repository-info":
    print am.getRepositoryInfo()
    
  elif args[0] == "--start-recording":
    if len(args) > 3:
      print am.startRecording(args[1], args[2], args[3])
    elif len(args) > 2:
      print am.startRecording(args[1], args[2], True)
    else:
      print am.startRecording(args[1], '__defaultArchive__', True)
      
  elif args[0] == "--stop-recording":
    print am.stopRecording(args[1])
    
  elif args[0] == "--move-archive":
    if len(args) > 2:
      print am.moveArchive(args[1], args[2])
    else:
      print am.moveArchive(args[1])
