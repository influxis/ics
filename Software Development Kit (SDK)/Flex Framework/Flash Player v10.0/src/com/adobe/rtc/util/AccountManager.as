   ...
   public class  AccountManager extends EventDispatcher
	{
		 ...
		 * @private
		 */
		protected var _templateMovePendingName:String;
		/**
		 * @private
		 */
		 ...
		 * @private
		 */
		protected var _archiveDeletePendingName:String;	
		/**
		 * @private
		 */
		protected var _archiveMovePendingName:String;			
		/**
		 * @private
		 */
		...
		/**
		 * Deletes a specified archive from the service.
		 *  
		 * @param p_archiveName The name of the archive to delete (archive name only--not the entire path).
		 * @param p_templateName Optional. The name of the template from which the archive came. 
		 * Including the archive name causes the ARCHIVE_DELETE
		 * event to include the <code>templateName</code>; ommitting <code>archiveName</code> has no effect.
		 */
		public function deleteArchive(p_archiveName:String, p_templateName:String=null):void
		{
			if (!isAuthenticated) {
				throw new Error("You must be authenticated to delete the archive.");
				return;
			}
			_archiveDeletePendingName = p_archiveName;
			_templateDeletePendingName = p_templateName;
			_meetingInfo.deleteItem(p_archiveName, MeetingInfoService.ARCHIVE_ITEMS);
		}
		 ...
		 *  
		 * @param p_archiveName The name of the archive to delete (archive name only--not the entire path).
		 * @param p_templateName Optional. The name of the template to which to move the archive. 
		 * Including the archive name causes the ARCHIVE_MOVE
		 * event to include the <code>templateName</code>; ommitting <code>archiveName</code> has no effect.
		 */
		public function moveArchive(p_archiveName:String, p_templateName:String=null):void
		{
			if (!isAuthenticated) {
				throw new Error("You must be authenticated to move the archive.");
				return;
			}
			_archiveMovePendingName = p_archiveName;
			_templateMovePendingName = p_templateName;
			_meetingInfo.modifyItemTemplate(p_archiveName, p_templateName, MeetingInfoService.ARCHIVE_ITEMS);
		}
		...
		/**
		 * Deletes the specified template. Deleting a template results in all the rooms and archives based on that
		 * template being moved to the default template. However the room's or archives's configuration does not change.
		 * NOTE: This API does not change the room node's template name.  The fact that room is now referring to an
		 * invalid template will make the dev portal show it underneath the default template.  If one is ever to recreate
		 * the deleted template, all the rooms and archives will reappear underneath it.
		 */
		...
		protected function onItemsReceive(p_evt:MeetingInfoEvent):void
		{
			...
			} else if (_archiveDeletePendingName) {
				e = new AccountManagerEvent(AccountManagerEvent.ARCHIVE_DELETE);
				e.archiveName = _archiveDeletePendingName;
				e.templateName = _templateDeletePendingName;
				dispatchEvent(e);
				_archiveDeletePendingName = null;
			} else if (_archiveMovePendingName) {
				e = new AccountManagerEvent(AccountManagerEvent.ARCHIVE_MOVE);
				e.archiveName = _archiveMovePendingName;
				e.templateName = _templateMovePendingName;
				dispatchEvent(e);
				_archiveMovePendingName = null;				
