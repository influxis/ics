...
package com.adobe.coreUI.controls.whiteboardClasses.shapes
{
	 ...
	 * @private
	 */
   public class  WBPropertiesToolBar extends UIComponent implements IWBPropertiesToolBar
	{
		[Embed (source = '../whiteboardAssets/toolBarIcons.swf#icon_fill')]
		protected static var _fillIconClass:Class;
		[Embed (source = '../whiteboardAssets/toolBarIcons.swf#icon_stroke')]
		protected static var _strokeIconClass:Class;
		[Embed (source = '../whiteboardAssets/toolBarIcons.swf#icon_stroke_thickness')]
		protected static var _thicknessIconClass:Class;
		[Embed (source = '../whiteboardAssets/toolBarIcons.swf#icon_alpha')]
		...
		protected var _thicknessCombo:ComboBox;
		...
		protected var _isFilledShape:Boolean = true;
		...
		public function get propertyData():*
		{
			...
			returnObj.lineThickness = _thicknessCombo.selectedItem;
		...
		{
			if (p_fill!=_isFilledShape) {
				_isFilledShape = p_fill;
		...
		override protected function commitProperties():void
		{
			if (_invPropsChanged) {
				...
				_thicknessCombo.dataProvider = (_isFilledShape) ? [0,1,2,4,6,8,10] : [1, 2, 5, 10, 15, 20];
				_thicknessCombo.selectedItem = _propertyData.lineThickness;
		...
		protected function createControls():void
		{
			...
			tmpIcon = new _thicknessIconClass();
			...
			_thicknessCombo.dataProvider = [0,1,2,4,6,8,10];
			_thicknessCombo.addEventListener(Event.CHANGE, fireEvent);
			addChild(_thicknessCombo);
			_thicknessCombo.validateNow();
			_thicknessCombo.setActualSize(_thicknessCombo.minWidth, _thicknessCombo.measuredHeight);
			maxHeight = Math.max(_thicknessCombo.height, maxHeight);
			...
			_alphaCombo.setActualSize(_alphaCombo.minWidth + 2, _alphaCombo.measuredHeight);
		...
		override protected function updateDisplayList(p_w:Number, p_h:Number):void
		{
			...
			_thicknessCombo.x = theX;
			_thicknessCombo.toolTip = (_isFilledShape) ? _lm.getString("Border Thickness") : _lm.getString("Line Thickness");
			_thicknessCombo.validateNow();
			_thicknessCombo.setActualSize(_thicknessCombo.minWidth, _thicknessCombo.measuredHeight);
			theX += _thicknessCombo.width + CONTROL_PADDING;
			_alphaIcon.x = theX;
