...
package com.adobe.rtc.collaboration
{
   ...
   public class  WebcamPublisher extends EventDispatcher implements ISessionSubscriber
	 {
		...
		* @private
		*/		 
		protected var _quality:Number = 80;
		
		/**
		* @private
		*/		 
		protected var _keyFrameInterval:Number = 48;
		...
		*/		 
		 ...
		 * @private
		 */
		protected var _resolutionW:int = 128;

		/**
		 * @private
		 */
		protected var _resolutionH:int = 128;
		 ...
		 * @private
		 */
		 protected var _cameraSettingChanged:Boolean = false ;
		 ...
		 * @private
		 */
		protected var _hackInterval:Timer;
		/**
		 * @private
		 */
		 protected var _multicastWindowDuration:int = 4 ;
		/**
		 * @private
		 */
		 protected var _bandwidth:Number = 0;
	    ...
	    */
		 ...
		 * given other factors like quality. Default is 0, which means the publisher will use as much bandwidth as it can to achieve the quality and FPS requested.
		...
		/**
		 * Specifies the number of pixels of width to capture from the camera. Note that the more pixels you capture, the more bandwidth will be consumed.
		 * @default 128
		 */
		public function get resolutionWidth():int
		{
			return _resolutionW;
		}
		 ...
		 * @private
		 */
		public function set resolutionWidth(p_w:int):void
		{
			if ( _resolutionW == p_w ) 
				return ;
		...
		/**
		 * Specifies the number of pixels of height to capture from the camera. Note that the more pixels you capture, the more bandwidth will be consumed.
		 * @default 128
		 */
		public function get resolutionHeight():int
		{
			return _resolutionH;
		}
		 ...
		 * @private
		 */
		public function set resolutionHeight(p_h:int):void
		{
			if ( _resolutionH == p_h ) 
				return ;
		...
		{
			if ( _streamManager.getStreamDescriptor(StreamManager.CAMERA_STREAM,_userManager.myUserID,_groupName)== null ) {
				return ;
			}
		...
		public function publish(p_publisherID:String=null):void
		{
			...
			if(p_publisherID==null || p_publisherID == _userManager.myUserID){
				...
				if(_streamManager.getUserRole(p_publisherID, StreamManager.CAMERA_STREAM,_groupName) < _streamManager.getNodeConfiguration(StreamManager.CAMERA_STREAM,_groupName).publishModel) {
					_streamManager.setUserRole(p_publisherID,
						_streamManager.getNodeConfiguration(StreamManager.CAMERA_STREAM,_groupName).publishModel,
						StreamManager.CAMERA_STREAM,_groupName);
				}
		...
		protected function updateCameraSettings():void
		{
			...
			_camera.setMode(_resolutionW, _resolutionH, _fps);
...
//			setCameraQuality();	
		...
		protected function createMyStream(p_finishedPublishing:Boolean=false, p_userID:String=null):void
		{
			...
			if ( !p_finishedPublishing ) {
				...
				streamDescriptor.nativeHeight = _resolutionH ;
				streamDescriptor.nativeWidth = _resolutionW ;
        ...
        */
        protected function onInvalidate(p_evt:Event=null):void
        {
        	...
        	if ( _cameraSettingChanged ) {
        		updateCameraSettings();
        		// invalidateDisplayList() ;
        		_cameraSettingChanged = false ;
        	}
        ...
        */
        protected function createNetStream():NetStream
	    {
	    	if ( _streamManager.isP2P ) {
				...
				if (!_hackInterval) {
					_hackInterval = new Timer(2000);
					_hackInterval.addEventListener(TimerEvent.TIMER,sendSnapShotPermission);
				}
				_hackInterval.start();
		 ...
		 * @private
		 */
		protected function sendSnapShotPermission(p_evt:Event=null):void
		{
			if (_stream) {
				_stream.send("|RtmpSampleAccess", true, true);
			}
