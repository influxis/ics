   ...
   public class  MessageManager extends EventDispatcher implements ISessionSubscriber
	{
		...
		*/
		...
		messaging_internal function configureNode(p_collectionName:String, p_nodeName:String, p_nodeConfiguration:NodeConfiguration):void
		{
			...
			if ( p_nodeConfiguration.p2pDataMessaging ) {
				if ( p_collectionName == FileManager.COLLECTION_NAME 
					|| p_collectionName == RoomManager.COLLECTION_NAME 
					|| p_collectionName == UserManager.COLLECTION_NAME ) {
					throw new Error("MessageManager.configureNode : cannot send P2P Data over shared Manager nodes");
					return;
				}
		...
		messaging_internal function publishItem(p_collectionName:String, p_nodeName:String, p_item:MessageItem, p_overWrite:Boolean=false):void
		{
			...
			var p2pDataMessaging:Boolean = false ;
			...
			if (cNode.isNodeDefined(p_nodeName)) {
				...
				if ( nodeConf.p2pDataMessaging != p2pDataMessaging ) {
					p2pDataMessaging = nodeConf.p2pDataMessaging ;
				}
			...
			_sessionManager.session_internal::publishItem(p_collectionName, p_nodeName, p_item.createValueObject(), p_overWrite,p2pDataMessaging);
		}
