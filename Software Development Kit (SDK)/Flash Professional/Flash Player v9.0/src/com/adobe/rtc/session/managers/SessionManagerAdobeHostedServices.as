...
package com.adobe.rtc.session.managers
{
	...
	import com.adobe.rtc.util.URLUtil;
   ...
   public class  SessionManagerAdobeHostedServices extends SessionManagerFMS
	{
		...
		protected function onMeetingInfo(p_evt:MeetingInfoEvent):void
		{
			...
			_sUTicket.sendRequest(getInstancePath(_meetingInfo.roomName), null, _meetingInfo.baseURL, _authenticator.authenticationKey); //it will be null the first time
		}
        ...
        {     
			...
			*/
            ...
            for (var i:uint=0; i<a.length; i++) {
            	...
            	if (protoPort[0]=="rtmfp" && majorVersion < 10) {
            		continue;
            	}
		...
		/*
		 * This is used by SessionManagerPlayback to set the instance
		 * as roomName + archiveID
		 */
		protected function getInstancePath(p_name: String):String
		{
			return p_name;
		}
        ...
        protected function connectToFMS():void
        {
			...
			if (_meetingInfo != null) {
				_fmsConnector.appInstanceName = getInstancePath(_meetingInfo.roomName);
			}
