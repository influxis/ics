...
package com.adobe.rtc.sharedManagers
{
	...
	import com.adobe.rtc.util.UtilArrayCollection;
	import com.adobe.rtc.util.Sort;
	 ...
	 *	&lt;session:ConnectSessionContainer id="cSession" roomURL="http://connect.acrobat.com/fakeRoom/" authenticator="{auth}"&gt;
	 *	&lt;/session:ConnectSessionContainer&gt;
	 * 
   ...
   public class  UserManager extends EventDispatcher implements ISessionSubscriber
	{
		...
		*/
		 ...
		 */
		public var userCollection:UtilArrayCollection;
		...
		*/
		...
		{
			userCollection = new UtilArrayCollection();
			_sorter = new Sort();
			_sorter.compareFunction = compareFunction;
			
			hostCollection = new UtilArrayCollection();
			participantCollection = new UtilArrayCollection();
			audienceCollection = new UtilArrayCollection();
		}
...
//			_myUserDescriptor = _userDescriptorTable[_myUserID] as UserDescriptor;
		 ...
		 * @param p_name The new displayName to assign to that user
		 */
		public function setPeer(p_userID:String, p_isPeer:Boolean):void
		{
			if (!_umCollectionNode.canUserPublish(myUserID, NODENAME_USER_LIST)) {
				throw new Error("UserManager.setUserStatus: insufficient permissions to change user name");
				return;
		 ...
		 */
		public function isPeerEnable():Boolean
		{
			for ( var i:int = 0 ; i < userCollection.length ; i++ ) {
				var userDesc:UserDescriptor = userCollection.getItemAt(i) as UserDescriptor ;
				...
				if (_myUserDescriptor.role == UserRoles.LOBBY) {
					...
					if ( (_connectSession.sessionInternals.session_internal::connection as NetConnection) != null ) {
						...
						if ( uri.split(":")[0] == "rtmfp" ) {
							_myUserDescriptor.isPeer = true ;
							_myUserDescriptor.isRTMFP = true ;
						}else {
							DebugUtil.debugTrace("Setting isPeer to false as connection is not RTMFP for User" + _myUserDescriptor.displayName);
							_myUserDescriptor.isPeer = false ;
							_myUserDescriptor.isRTMFP = false 
						}
					}else {
						DebugUtil.debugTrace("Setting isPeer to false as NetConnection is null for User" + _myUserDescriptor.displayName);
						_myUserDescriptor.isPeer = false ;
						_myUserDescriptor.isRTMFP = false ;
					}
		...
		protected function userReceivedOrEdited(p_evt:CollectionNodeEvent):void
		{
			...
			if (oldDesc!=null) {
				...
				if (oldDesc.isPeer != userDesc.isPeer) {
					removeUserFromCollection(oldDesc);
					addUserToCollection(userDesc);
					//TODO: we might need to add a event for this...
					dispatchEvent(new UserEvent(UserEvent.PEER_CONNECTION_CHANGE, userDesc));
				}
			} else {
				// it's completely new
				_userDescriptorTable[userDesc.userID] = userDesc;
